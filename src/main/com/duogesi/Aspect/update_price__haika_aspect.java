...
import com.duogesi.entities.huodai.price_haika;
...
public class update_price__haika_aspect {
     ...
     */
    @Pointcut("execution(public * com.duogesi.service.Huodaiservice.insert_haika(..)) ")
    ...
    public int aroundMethod(ProceedingJoinPoint point){
        ...
        //获取切点方法的参数
        price_haika price_haika= (price_haika) args.get(0);
        int user_id=(int)args.get(1);
        try {
            //前置通知
...
//            System.out.println("The method "+ methodName+" end. result<"+ result+">");
        ...
        Boolean res=true;
        if(price_haika.getValid_date_start().before(price_haika.getValid_date_end())){
            try {
               List<price_haika> price_haikas=priceMapper.check_ship_noincludetax(price_haika.getQiyungang(),price_haika.getMudigang(),price_haika.getUser_id());
                for (price_haika p:price_haikas
                     ) {
                    Date end=p.getValid_date_end();
                    Date start=p.getValid_date_start();
                    Date valid_end=price_haika.getValid_date_end();
                    Date valid_start=price_haika.getValid_date_start();
               ...
               List<Ship> list = shipMapper.get_before_update_rate_haika(price_haika.getValid_date_start(), price_haika.getValid_date_end(),user_id,price_haika.getCountry());
                if (list.size() == 0&&res) {
                   point.proceed();
                   return 1;
                } else return 3;
