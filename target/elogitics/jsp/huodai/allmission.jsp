...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            var deleted="<%=request.getParameter("delete")%>";
            if(deleted!="null"){
                alert(deleted)
            }
            var the="<%=request.getParameter("the")%>";
            if(the!="null"){
                alert(the)
            }
        })
    ...
    <div class="main">
            ...
            <table class="table">
                ...
                <c:forEach items="${pagebean.list}" var="o" >
                    <tr >
                        ...
                        <td>${o.desc}</td>
                        <td>${o.statu}</td>
                        <td><a  href = "<%=path%>/huodai/thedetails.do?id=${o.id}&&page=0&&number=${o.number}" class="a_post">更多</a>&nbsp;&nbsp;&nbsp;&nbsp;<a onclick="return confirm('确认删除?')" href="<%=path%>/huodai/delete_order.do?number=${o.number}&&id=${o.id}">删除</a></td>
                        <td><p style="color: red;font-size: 15px">${o.manbipei}</p></td>
                    </tr>
                </c:forEach>
                </table>
            </div>
        <div>
            <form action="<%=path%>/huodai/check_schdule_id.do" METHOD="post" style="position: absolute;left:15%">
        ...
        <div style="position: relative;margin-left: 65% " >
            ...
            <c:choose>
                ...
                <c:otherwise>
                    <a href="<%=path%>/huodai/allmission.do?page=1">首页</a>
                    &nbsp;&nbsp;&nbsp;
                    <a href="<%=path%>/huodai/allmission.do?page=<c:out value="${pagebean.currentPage-1}" /> ">上一页</a>
                </c:otherwise>
            </c:choose>
            <c:choose>
                ...
                <c:otherwise>
                    <a href="<%=path%>/huodai/allmission.do?page=<c:out value="${pagebean.currentPage+1}" />">下一页</a>
                    &nbsp;&nbsp;&nbsp;
                    <a href="<%=path%>/huodai/allmission.do?page=${pagebean.totalPage}">尾页</a>
                </c:otherwise>
            </c:choose>
            <p style="display: inline-block;">
            <form action="<%=path%>/huodai/allmission.do?" size="2" onclick="return validate(this)">
